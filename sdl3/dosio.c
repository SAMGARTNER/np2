...
void file_setcd(const char *exepath) {
    printf("setting path for BIOS FONT SOUND %s\n", exepath);
//	file_cpyname(curpath, exepath, sizeof(curpath));
//	curfilep = file_getname(curpath);
//	*curfilep = '\0';

    file_cpyname(curpath, exepath, sizeof(curpath));
    file_cutseparator(curpath);  // Remove trailing '/' if any
    file_setseparator(curpath, sizeof(curpath));  // Add trailing '/'
    curfilep = curpath + strlen(curpath);
    printf("path for BIOS FONT SOUND %s\n", curpath);
}
